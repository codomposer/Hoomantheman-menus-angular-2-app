...
const key = 'innerWidth_gt_SCREEN_MD_MIN$'
export const innerWidth_gt_SCREEN_MD_MIN$_b:B<dom_Ctx, typeof key> = _b(key, (ctx)=>{
	const innerWidth$ = innerWidth$_b(ctx)
	return derived$(innerWidth$, innerWidth=>
		innerWidth > SCREEN_MD_MIN
	)
})
export type innerWidth_gt_SCREEN_MD_MIN$_T = Readable$<boolean>
