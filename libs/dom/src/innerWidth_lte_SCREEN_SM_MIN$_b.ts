...
const key = 'innerWidth_lte_SCREEN_SM_MIN$'
export const innerWidth_lte_SCREEN_SM_MIN$_b:B<dom_Ctx, typeof key> = _b(key, (ctx)=>{
	const innerWidth$ = innerWidth$_b(ctx)
	return derived$(innerWidth$, innerWidth=>
		innerWidth <= SCREEN_SM_MIN
	)
})
export type innerWidth_lte_SCREEN_SM_MIN$_T = Readable$<boolean>
