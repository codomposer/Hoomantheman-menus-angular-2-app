...
const key = 'innerWidth_lte_SCREEN_XS_MIN$'
export const innerWidth_lte_SCREEN_XS_MIN$_b:B<dom_Ctx, typeof key> = _b(key, (ctx)=>{
	const innerWidth$ = innerWidth$_b(ctx)
	return derived$(innerWidth$, innerWidth=>
		innerWidth <= SCREEN_XS_MIN
	)
})
export type innerWidth_lte_SCREEN_XS_MIN$_T = Readable$<boolean>
