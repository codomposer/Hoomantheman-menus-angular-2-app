...
const Controller_name = 'Search_c'
export class Search_c extends BaseController<search_ui_Ctx> {
	...
	readonly busyDish$:Writable$<boolean> = writable$(false)
	...
	readonly search_dishes_a$:Writable$<any[]> = writable$([])
	...
	dishRequestData = new SearchMenuRequestQuery()
	...
	readonly search_dishes_if_needed = async () => {
		// Only load dishes if dish filters are enabled
		if(this.platform_settings.$.Enable_Dish_Filter) {
			const requestData = this.create_get_restaurants_request()

			if(this.isRequestChanged(this.dishRequestData, requestData)) {
				this.busyDish$.$ = true
				try {
					this.dishRequestData = requestData

					const dishesPayload = await this.consumer_http_client.search_dish(clone(requestData))

					this.search_dishes_a$.$ = dishesPayload.Data
				} finally {
					this.busyDish$.$ = false
				}
			}
	...
	onDishSelected = async ({ detail: dishItems }) => {
		// Since it is single select, so it should return only 1 selected item
		if(dishItems && dishItems.length === 1) {
			const dishItem = dishItems[0]

			await this.navigating_goto([
				'/menu',
				query_str_(
					{
						keywords: dishItem.DishName.toLowerCase()
					}
				)
			])
		}
