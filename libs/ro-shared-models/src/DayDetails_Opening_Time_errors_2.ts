...
export function DayDetails_Opening_Time_errors_2(list:RestHours[]):DayDetails_Opening_Time_errors__T {
	return (DayDetails:HourDayDetails)=>{
		...
		const DayDetailsOpening = `${DayDetails.dayIDFrom}:${Opening_Time}`
		for (const currDay of list) {
			for (const currDay_DayDetails of currDay.DayDetails) {
				if (DayDetails.ID !== currDay_DayDetails.ID) {
					...
					if (DayDetailsOpening >= currDay_Opening_str && DayDetailsOpening <= currDay_Closing_str) {
						errors.push(
...
export type DayDetails_Opening_Time_errors__T = (DayDetails:HourDayDetails)=>string[]
