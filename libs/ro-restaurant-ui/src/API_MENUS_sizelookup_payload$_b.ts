...
import { API_MENUS_sizelookup_payload_T, ro_httpClient_b, RoRequestQuery, } from '@menus/ro-http'
...
const key = 'API_MENUS_sizelookup_payload$'
export const API_MENUS_sizelookup_payload$_b:B<ro_restaurant_ui_Ctx, typeof key> = be_(key, ctx=>{
	const invalidate_signal$ = writable$(undefined)
	let current_invalidate_signal:any
	const API_MENUS_sizelookup_payload$ = derived$(
		...
		([ro_login_user, params_fireFlyID, invalidate_signal],
		 set
			...
			run(async ()=>{
				...
				const payload = await ro_httpClient_b(ctx).API_MENUS_sizelookup(requestData)
				if (
	...
	return assign(API_MENUS_sizelookup_payload$, {
		invalidate() {
			current_invalidate_signal = {}
			invalidate_signal$.$ = current_invalidate_signal
		},
		is_ERR_INVALID_ACCESS$: is_ERR_INVALID_ACCESS$_(API_MENUS_sizelookup_payload$ as Readable$<API_error_T>)
	}) as API_MENUS_sizelookup_payload$_T
})
export interface API_MENUS_sizelookup_payload$_T
	extends Readable$<API_MENUS_sizelookup_payload_T>,
