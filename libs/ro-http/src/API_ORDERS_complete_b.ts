...
const key = 'API_ORDERS_complete'
export const API_ORDERS_complete_b:B<ro_http_Ctx, typeof key> = be_(key, (ctx)=>{
	const ro_login_user$ = ro_login_user$_b(ctx)
	const ro_fetch = ro_fetch_b(ctx)
	return API_ORDERS_complete as API_ORDERS_complete_T
	async function API_ORDERS_complete(requestData:RoRequestQuery_I) {
		RoRequestQuery.fill_login_user(requestData, ro_login_user$.$)
		requestData.qt = 'complete'
...
export type API_ORDERS_complete_T = (requestData:RoRequestQuery_I)=>Promise<API_ORDERS_accept_payload_T>
