...
export function min_errors_2(min:any, opts:compare_opts_T = {}):errors__T {
	return (value:any, label = '')=>{
		if ((typeof value !== 'number' && !value) || (typeof min !== 'number' && !min)) return []
		...
		return (
			compare_fn(preprocess_fn(value), min)
			? []
			: [`${label_str_(label)}must be at least ${compare_error_text_fn(min)}`])
	}
}
